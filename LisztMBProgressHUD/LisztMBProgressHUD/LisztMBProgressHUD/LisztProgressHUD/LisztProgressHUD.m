...
#define LisztMainThreadAssert() NSAssert([NSThread isMainThread], @"LisztProgressHUD needs to be accessed on the main thread.");

CGFloat const LisztProgressMaxOffset = 1000000.f;

static const CGFloat LisztDefaultPadding = 4.f;
static const CGFloat LisztDefaultLabelFontSize = 16.f;
static const CGFloat LisztDefaultDetailsLabelFontSize = 12.f;


@interface LisztProgressHUD ()
...
@interface LisztProgressHUDRoundedButton : UIButton
@end


@implementation LisztProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    LisztProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    LisztProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (LisztProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            LisztProgressHUD *hud = (LisztProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = LisztProgressHUDAnimationFade;
    _mode = LisztProgressHUDModeIndeterminate;
    _margin = 20.0f;
...
- (void)animateIn:(BOOL)animatingIn withType:(LisztProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == LisztProgressHUDAnimationZoom) {
        type = animatingIn ? LisztProgressHUDAnimationZoomIn : LisztProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == LisztProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == LisztProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == LisztProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == LisztProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
...
- (void)done {
    ...
    LisztProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<LisztProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    LisztBackgroundView *backgroundView = [[LisztBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = LisztProgressHUDBackgroundStyleSolidColor;
    ...
    LisztBackgroundView *bezelView = [LisztBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:LisztDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:LisztDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [LisztProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:LisztDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[LisztRoundProgressView class]];

    LisztProgressHUDMode mode = self.mode;
    if (mode == LisztProgressHUDModeIndeterminate) {
    ...
    else if (mode == LisztProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[LisztBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == LisztProgressHUDModeDeterminate || mode == LisztProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[LisztRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == LisztProgressHUDModeAnnularDeterminate) {
            [(LisztRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == LisztProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == LisztProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[LisztProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[LisztProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[LisztRoundProgressView class]]) {
        LisztRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [LisztRoundProgressView appearanceWhenContainedIn:[LisztProgressHUD class], nil];
#else
        appearance = [LisztRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[LisztProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((LisztRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((LisztRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[LisztBarProgressView class]]) {
        LisztBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [LisztBarProgressView appearanceWhenContainedIn:[LisztProgressHUD class], nil];
#else
        appearance = [LisztBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[LisztProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((LisztBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((LisztBarProgressView *)indicator).lineColor = color;
        }
    } else {
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    LisztBackgroundView *bezelView = self.bezelView;
...
- (void)layoutSubviews {
    ...
    // would trigger a zoLisztie object access.
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? LisztDefaultPadding : 0.f;
...
- (void)setMode:(LisztProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == LisztProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
- (void)updateForCurrentOrientationAnimated:(BOOL)animated {
    ...
    BOOL iOS8OrLater = kCFCoreFoundationVersionNuLiszter >= kCFCoreFoundationVersionNuLiszter_iOS_8_0;
...
@implementation LisztRoundProgressView
...
@implementation LisztBarProgressView
...
@implementation LisztBackgroundView

#pragma mark - Lifecycle

- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        _style = LisztProgressHUDBackgroundStyleBlur;
...
- (void)setStyle:(LisztProgressHUDBackgroundStyle)style {
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
    }
...
- (void)updateForBackgroundStyle {
    ...
    LisztProgressHUDBackgroundStyle style = self.style;
    if (style == LisztProgressHUDBackgroundStyleBlur) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == LisztProgressHUDBackgroundStyleBlur) {
        self.backgroundColor = self.color;
    } else {
        self.backgroundColor = self.color;
    }
...
@implementation LisztProgressHUDRoundedButton
